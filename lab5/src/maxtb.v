...
`define TDATA_FILE "/home/ead/yihui/ece6250/lab5/mxtb.dat"
`ifndef XTB_DUT_HIER 
...
`define test_so 18
`define _pi 19
`define all_inputs 20
`define _po 21
`define _si 22
`define all_outputs 23
`define _clk 24
`define _so 25
...
module /* maxtb */ ripplecarry4_clk_test ();
   ...
   parameter MAXCHLEN = 3, NCHAINS = 1; // MAXCHLEN for serial
   parameter NINPUTS = 13, NOUTPUTS = 6, CUM_WIDTH=60;
   parameter NSIS = 1, NSOUTS = 1;
   parameter WFT_IDS = 2, SIG_IDS = 26, SIG_IDS_W = 5, MAX_WIDTH = 13, MAX_SIGW = 1;
   parameter PARSERINC = 3; 
   ...
   reg [8*NAMELENGTH-1:0]  SignalIDName [0:25];
   reg [2:0]  SignalIDType [0:25];
   reg [3:0]  SignalIDWidth [0:25];
   ...
   begin
     if (SignalIDType[sid] != 3) the_sids[0] = sid;
       ...
       case (sid)
         'd19: begin // _pi
                 the_sids[12] = 3; the_sids[11] = 2; the_sids[10] = 1; the_sids[9] = 0; the_sids[8] = 7; the_sids[7] = 6; the_sids[6] = 5; the_sids[5] = 4; the_sids[4] = 8; the_sids[3] = 9; the_sids[2] = 10; the_sids[1] = 11;
                 the_sids[0] = 12;
               end
         'd20: begin // all_inputs
                 the_sids[12] = 3; the_sids[11] = 2; the_sids[10] = 1; the_sids[9] = 0; the_sids[8] = 7; the_sids[7] = 6; the_sids[6] = 5; the_sids[5] = 4; the_sids[4] = 8; the_sids[3] = 9; the_sids[2] = 10; the_sids[1] = 11;
                 the_sids[0] = 12;
               end
         'd21: begin // _po
                 the_sids[5] = 17; the_sids[4] = 16; the_sids[3] = 15; the_sids[2] = 14; the_sids[1] = 13;
                 the_sids[0] = 18;
               end
         'd22: begin // _si
   ...
   begin
     case (sid)
       'd13:	strsig[0:0] = 	\sum[3]  ;
       'd14:	strsig[0:0] = 	\sum[2]  ;
       'd15:	strsig[0:0] = 	\sum[1]  ;
       'd16:	strsig[0:0] = 	\sum[0]  ;
       'd17:	strsig[0:0] = 	cout_con ;
       'd18:	strsig[0:0] = 	test_so_con ;
   ...
   always  begin: force_sigs_p
     ...
     fork
        ...
        forever @(ALLINSIGS[10]) set_force_sig('d`rst );
        forever @(ALLINSIGS[11]) set_force_sig('d`test_si );
        forever @(ALLINSIGS[12]) set_force_sig('d`test_se );
     join
   end
   ...
   always begin: measure_sigs_p
    fork
     forever @(ALLOUTSIGS['d`\sum[3] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `multiclock_captureStmt ) measure_out_sig('d`\sum[3] ,ALLOUTSIGS[13], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\sum[2] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `multiclock_captureStmt ) measure_out_sig('d`\sum[2] ,ALLOUTSIGS[14], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\sum[1] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `multiclock_captureStmt ) measure_out_sig('d`\sum[1] ,ALLOUTSIGS[15], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`\sum[0] ]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `multiclock_captureStmt ) measure_out_sig('d`\sum[0] ,ALLOUTSIGS[16], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`cout]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `multiclock_captureStmt ) measure_out_sig('d`cout,ALLOUTSIGS[17], cur_stmt); 
     end
     forever @(ALLOUTSIGS['d`test_so]) begin
       #0 if (cur_stmt == `VectorStmt || cur_stmt == `load_unloadStmt || cur_stmt == `multiclock_captureStmt )
       if (cur_stmt ==`load_unloadStmt) measure_scan_out('d`test_so,ALLOUTSIGS[18], 0); 
       else measure_out_sig('d`test_so,ALLOUTSIGS[18], cur_stmt); 
     end
    join
   end 
   ...
   begin
     for (sid=0; sid < SIG_IDS; sid=sid+1) begin
       n=0; 
       case(sid)
         'd3, 'd2, 'd1, 'd0, 'd7, 'd6, 'd5, 'd4, 'd8, 'd10, 'd11, 'd12: // \a[0] , \a[1] , \a[2] , \a[3] , \b[0] , \b[1] , \b[2] , \b[3] , cin, rst, test_si, test_se_con
         begin
         ...
         'd17, 'd16, 'd15, 'd14, 'd13, 'd18: // cout, \sum[0] , \sum[1] , \sum[2] , \sum[3] , test_so_con
         begin
  ...
  endtask
   ...
   begin
     ...
     //check_sid_args("test_setup",26'b1, idargs);
     resolve_wft(1);

     assign_val('d`all_inputs , 26'b0000000000000XXXXXXXXXXXXX);
   ...
   begin
    ...
    while (csci < 3) begin
     ...
     assign_val('d`_clk , 2'bZ1);
         if (idargs[22] === 1'b1 || idargs[11] === 1'b1) begin 
           LOD=LOAD[0]; assign_val('d`test_si , {1'b0, LOD[csci+0]});
         end
         if(stroben && (idargs[25] === 1'b1 || idargs[18] === 1'b1)) begin 
   ...
   always /* ParallelShiftMode */ @(strobe_scells) begin
     CHAINOUT0[0:2] = {`XTB_DUT_HIER.test_so , `XTB_DUT_HIER.c2_reg_reg.TI , `XTB_DUT_HIER.c1_reg_reg.TI };
   end

   `define CELL0IN0 `XTB_DUT_HIER.c0_reg_reg.TI
   `define CHAININ0 {`XTB_DUT_HIER.c2_reg_reg.TI , `XTB_DUT_HIER.c1_reg_reg.TI ,  `CELL0IN0 }
   ...
   task p_shift_0 ;
     ...
     integer i, j;
   begin
     if (~(idargs[22] | idargs[11] | idargs[25] | idargs[18] )) 
       disable p_shift_0 ; 
     ...
     fork 
       begin: b1 
         ...
         if (idargs[25] === 1'b1 ||  idargs[18] === 1'b1) begin 
   ...
   begin
     ...
     assign_val('d`all_inputs , 26'b0000000000000XXXXXXXXX0XXX);
     assign_val('d`all_outputs , 12'b111111XXXXXX);
   ...
   begin
     ...
     //check_sid_args("multiclock_capture",26'b1, idargs);
     save_context;
     resolve_wft(0);
     ...
     if (idargs[19] === 1'b1) assign_ival('d`_pi );
     if (idargs[3] === 1'b1) assign_ival('d`\a[0]  );
     if (idargs[2] === 1'b1) assign_ival('d`\a[1]  );
     if (idargs[1] === 1'b1) assign_ival('d`\a[2]  );
     if (idargs[0] === 1'b1) assign_ival('d`\a[3]  );
     if (idargs[7] === 1'b1) assign_ival('d`\b[0]  );
     if (idargs[6] === 1'b1) assign_ival('d`\b[1]  );
     if (idargs[5] === 1'b1) assign_ival('d`\b[2]  );
     if (idargs[4] === 1'b1) assign_ival('d`\b[3]  );
     if (idargs[8] === 1'b1) assign_ival('d`cin );
     if (idargs[9] === 1'b1) assign_ival('d`clk );
     if (idargs[10] === 1'b1) assign_ival('d`rst );
     if (idargs[11] === 1'b1) assign_ival('d`test_si );
     if (idargs[12] === 1'b1) assign_ival('d`test_se );
     if (idargs[21] === 1'b1) assign_ival('d`_po );
     if (idargs[17] === 1'b1) assign_ival('d`cout );
     if (idargs[16] === 1'b1) assign_ival('d`\sum[0]  );
     if (idargs[15] === 1'b1) assign_ival('d`\sum[1]  );
     if (idargs[14] === 1'b1) assign_ival('d`\sum[2]  );
     if (idargs[13] === 1'b1) assign_ival('d`\sum[3]  );
     if (idargs[18] === 1'b1) assign_ival('d`test_so );
     assign_stmt(the_wft_id);
   ...
   initial begin: _burst__pattern__init_unit
     ...
     SignalIDName[10] = "rst";
     SignalIDType[10] = 0; //rst
     SignalIDWidth[10] = 1; //rst
     SignalIDName[11] = "test_si";
     SignalIDType[11] = 0; //test_si
     SignalIDWidth[11] = 1; //test_si
     SignalIDName[12] = "test_se";
     SignalIDType[12] = 0; //test_se
     SignalIDWidth[12] = 1; //test_se
     SignalIDName[13] = "sum[3]";
     SignalIDType[13] = 1; //\sum[3] 
     SignalIDWidth[13] = 1; //\sum[3] 
     SignalIDName[14] = "sum[2]";
     SignalIDType[14] = 1; //\sum[2] 
     SignalIDWidth[14] = 1; //\sum[2] 
     SignalIDName[15] = "sum[1]";
     SignalIDType[15] = 1; //\sum[1] 
     SignalIDWidth[15] = 1; //\sum[1] 
     SignalIDName[16] = "sum[0]";
     SignalIDType[16] = 1; //\sum[0] 
     SignalIDWidth[16] = 1; //\sum[0] 
     SignalIDName[17] = "cout";
     SignalIDType[17] = 1; //cout
     SignalIDWidth[17] = 1; //cout
     SignalIDName[18] = "test_so";
     SignalIDType[18] = 1; //test_so
     SignalIDWidth[18] = 1; //test_so
     SignalIDName[19] = "_pi";
     SignalIDType[19] = 3; //_pi
     SignalIDWidth[19] = 13; //_pi
     SignalIDName[20] = "all_inputs";
     SignalIDType[20] = 3; //all_inputs
     SignalIDWidth[20] = 13; //all_inputs
     SignalIDName[21] = "_po";
     SignalIDType[21] = 3; //_po
     SignalIDWidth[21] = 6; //_po
     SignalIDName[22] = "_si";
     SignalIDType[22] = 3; //_si
     SignalIDWidth[22] = 1; //_si
     SignalIDName[23] = "all_outputs";
     SignalIDType[23] = 3; //all_outputs
     SignalIDWidth[23] = 6; //all_outputs
     SignalIDName[24] = "_clk";
     SignalIDType[24] = 3; //_clk
     SignalIDWidth[24] = 1; //_clk
     SignalIDName[25] = "_so";
     SignalIDType[25] = 3; //_so
     SignalIDWidth[25] = 1; //_so
     ...
     `ifdef tmax_vcde
       $display("XTB: Dumping Extended VCD information in file \"mxtb.vcd\"");
       $dumpports(`XTB_DUT_HIER, "mxtb.vcd");
     `endif
